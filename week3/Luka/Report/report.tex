...
While 800 panels does clearly give the best result, those for 400 panels are already adequate, and given the $\mathcal{O}\left(n^3\right)$ matrix inversions required, the substantially greater computation time is not worth the accuracy increment.

Better results could very likely be obtained by using non-uniform panel lengths (which our code was adapted to support), to allow clustering around the leading and trailing edges, where there tend to be difficult-to-resolve large pressure gradients, while not wasting computational effort on midpoints. This non-uniform panelling (among other differences) allows Xfoil to obtain good quality results with only 150 or so panels \cite{xfoil}. 
...
\end{figure}
    ...
    \begin{minipage}{0.49\textwidth}
        \centering
        \includegraphics[width=\linewidth]{../../Figures/cp_NACA4412.eps}
    \end{minipage}
    \hfill
    \begin{minipage}{0.49\textwidth}
        \centering
...
Max $L/D$ of 152.1 at $\alpha=\qty{6.9}{\degree}$
\begin{figure}[H]
    \centering
    \begin{minipage}{0.49\textwidth}
        \centering
...
Regardless of the accuracy and validity of the performance results, aerofoils must meet other practical constraints. They must be manufacturable: the low-speed aerofoil needs a low-waviness and smooth surface finish to maintain laminar flow, which may not be suitable for all mnaufacturing techniques, and the high-speed aerofoil has a difficult to manufacture very thing trailing edge.

The thicknesses may also pose a challenge. The aerofoil must be sufficiently strong and stiff to resist regular bending and torsion loads, and must be stiff enough to give suitably high aeroelastic divergence and flutter speeds \cite{aeroelasticity}. Moreover, in civil transports, the section must be able to accommodate a ``wet wing'' fuel tank. These constraints all encourage a thicker section than we might otherwise select.
